...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        message.channel.send(new MessageEmbed().setTitle(await client.translate("Are you sure you want to reset Game-Log from your server?", message)).setColor('#fff5ee').setFooter('🔥 Niyuki On Fire 🔥')
        .setDescription(await client.translate("  if you want to remove the data please Click on ✅ or ❌ if you want to keep it", message))).then(async (msg) => {
            ...
            if(emoji === '✅') {
                Schema.findOne({ guild: message.guild.id}, async(err, data) => {
                    ...
                    await msg.edit(new MessageEmbed().setTitle(await client.translate("Succesfully", message)).setColor('#0dfa3b').setFooter('🔥 Niyuki On Fire 🔥').setDescription(await client.translate("**Removed your data from database!**")))
                });
            }
            if(emoji === '❌') {
