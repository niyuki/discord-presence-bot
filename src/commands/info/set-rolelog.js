...
module.exports = {
    name: 'rolelog',
    aliases: ['set-rolelog','set-rl'],
    description: 'Set Your Role Log Channel to show what roles are given to users!',
    /** 
     * @param {Client} client 
    ...
    run: async(client, message, args) => {
        ...
        Schema.findOne({ guild: message.guild.id}, async(err, data) => {
            if(data) {
                message.channel.send(new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, })).setDescription(await client.translate(`There is already a Role-Log saved. Do you want to change it?`, message)).setColor('#fff5ee').setFooter('🔥 Niyuki On Fire 🔥')).then(async (msg) => {
                    const emoji = await confirmation(msg, message.author, ["✅", "❌"], 30000);
                    if(emoji === '✅') {
                        msg.edit(new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, })).setColor('0dfa3b').setDescription(`${await client.translate(`✅ \n New Role-Log has been updated to`, message)} ${newchannel} by ${message.member}`).setFooter('🔥 Niyuki On Fire 🔥').setTimestamp())
                        data.delete()
                        new Schema({ 
